...
## 4.6 플랫폼별 동작 — ⚠️ Android 는 재생이 안 될 수 있음

보호 VOD(Cloudflare-TLS) 처리는 **플랫폼마다 다르다.**

| | Windows (exe) | Android (apk) |
|---|---|---|
| 보호 사이트 **재생** | ✅ yt-dlp 위장 프록시(번들/자동 다운로드) | ⚠️ **WebView 로 시도 — 사이트에 따라 안 될 수 있음** |
| 보호 사이트 **다운로드** | ✅ yt-dlp 위장 | ❌ 미지원 |
| yt-dlp 자동 다운로드 | ✅ (없으면 받아서 사용) | ❌ 불가(아래 사유) |
| 일반 링크/유튜브/빌트인 추출 | ✅ | ✅ |

**Android 에서 yt-dlp 자동 다운로드/실행이 불가능한 이유:**
1. `yt-dlp.exe` 는 Windows 전용 바이너리(Android 에서 실행 불가).
2. Android 10+ 는 **앱이 다운로드한 실행파일의 exec 를 차단**(W^X) — "받아서 실행" 자체가 막힘.
3. 차단은 TLS 지문이라 **curl_cffi(브라우저 위장)** 가 필요한데 Android 용 yt-dlp 엔 없음.

그래서 Android 는 **WebView(내장 브라우저, Chrome TLS)** 로 그 페이지를 직접 열어
사이트 자체 플레이어로 재생을 시도한다. 이는 best-effort 이며, 임베드 차단·로그인·
코덱 등으로 **사이트에 따라 재생되지 않을 수 있다.** 보호 사이트의 안정적 재생·
다운로드는 **데스크톱(Windows) 앱**을 사용한다.

## 4.7 스플래시 화면 (재생 엔진 준비)

앱 시작 시 스플래시에서 재생 엔진을 준비한 뒤 보드로 진입한다.
- **Windows**: 번들 yt-dlp 가 없으면 공식 빌드를 **진행률과 함께 다운로드**(최초 1회).
- **Android**: 준비할 yt-dlp 가 없으므로 상태만 표시하고 곧장 진입(보호 사이트는 4.6 의 WebView 경로).
스플래시는 길어야 Windows 1회 다운로드 시간만큼만 머문다.
